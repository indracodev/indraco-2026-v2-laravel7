...
@section('content')
   <section class="banner overflow-hidden">
      <div class="container-sm">
         <div class="row g-0">
            <div class="col-12">
               <h1 id="section-banner-title" class="banner-title z-0">DOWNLOADS</h1>
            </div>
            <div class="col-7 z-2">
               ...
               <div class="banner-media ratio ratio-1x1">
                  <div class="banner-media-images h-auto top-0 start-50 translate-middle z-1">
                     <div class="ratio ratio-1x1 w-100">
                        <img src="{{ asset('images/icon-download.png') }}" alt="" aria-hidden="true" loading="lazy"
      ...
      <div class="bg-body-secondary rounded-4 p-4 p-lg-5">
         ...
         <ul class="download-list list-unstyled mb-0 row g-5">
            @forelse($downloads as $item)
               <li class="download-item col-12 col-md-6 col-xl-4">
                  <article class="card h-100 bg-transparent border-0">
                     <div class="card-header ratio ratio-1x1 bg-body-tertiary border-0 shadow rounded-4">
                        <img src="{{ $item->image_url }}" alt="" aria-hidden="true" loading="lazy"
                           class="card-img object-fit-contain top-50 start-50 translate-middle h-75">
                        @if ($item->file_size)
                           <div class="m-3 small text-muted" id="file-size-{{ $loop->index }}">
                     ...
                     <div class="card-body text-center">
                        <h3 class="card-title fs-5 fw-semibold" id="title-{{ $loop->index }}">{{ $item->judul }}</h3>
                        @if ($item->judul_eng)
                           <p class="card-text" lang="en">{{ $item->judul_eng }}</p>
                        @endif
                     </div>
                     <a href="{{ $item->download_url }}" class="stretched-link"
                        aria-labelledby="action-{{ $loop->index }} title-{{ $loop->index }}"
                        @if ($item->file_size) aria-describedby="file-size-{{ $loop->index }}" @endif>
                        <span class="visually-hidden" id="action-{{ $loop->index }}">Unduh</span>
                     </a>
                  </article>
               </li>
            @empty
               <li class="download-item col-12">
